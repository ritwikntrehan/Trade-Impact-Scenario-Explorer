  ...
  return (
    <main className="section space-y-6">
      ...
      <p className="text-sm text-steel">
        What happens to {regions[0]?.name ?? "U.S."}{" "}
        {industry?.name.toLowerCase()} participants if tariffs on{" "}
        {country?.name} components rise by {scenario.tariffDeltaPct}%?
      </p>

      <div className="card p-5">
        <h2 className="font-semibold">Scenario setup</h2>
        <p className="mt-2 text-sm text-steel">
          {country?.name} · {industry?.name} · {scenario.mode} mode ·{" "}
          {scenario.year}
        </p>
      </div>
      ...
      <div className="card p-5">
        ...
        <ul className="mt-2 space-y-1 text-sm text-steel">
          {flows.map((f) => (
            <li key={f.id}>
              {f.productGroup}: import ${f.importExposureValue.toFixed(1)}B ·
              export ${f.exportExposureValue.toFixed(1)}B
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="card p-5">
        ...
        <ul className="mt-2 space-y-1 text-sm text-steel">
          {alternatives.map((a) => (
            <li key={a.id}>
              {a.alternativeCountryCode} ({a.relativeStrengthLabel}):{" "}
              {a.rationale}
            </li>
          ))}
        </ul>
      ...
      <div className="card p-5">
        ...
        <p className="mt-2 text-sm text-steel">
          Pressure direction: {pressureDirection(scenario.mode, scenario.tariffDeltaPct)}.
        </p>
      ...
      <Link
        ...
        className="inline-block rounded bg-ink px-4 py-2 text-sm text-white"
      >
